      ...
      <div class="singer-wrapper">
        <ScrollView ref="scrollView">
          <ul class="list-wrapper">
            <li class="list-group" v-for="(value, index) in artists" :key="index" ref="group">
              <h2 class="group-title">{{keys[index]}}</h2>
              <ul>
                <li class="group-item" v-for="obj in artists[index]" :key="obj.id" @click.stop="switchSinger(obj.id)">
        ...
        <ul class="list-keys">
          ...
          :class="{'active': currentNum === index}">{{key}}</li>
          -->
          <li
          v-for="(key, index) in keys"
          :key="key"
...
export default {
  ...
  computed: {
    fixTitle () {
      if (this.scrollY >= 0) {
        ...
        return this.keys[this.currentNum]
      }
    }
  },
  created () {
  ...
  mounted () {
    this.$refs.scrollView.scrolling((y) => {
      ...
      // 处理最后一个区域
      // this.currentNum = this.groupsTop.length - 1
    })
  },
  data () {
    return {
    ...
    artists () {
      this.$nextTick(() => {
        this.$refs.group.forEach(element => {
          this.groupsTop.push(element.offsetTop)
        })
